...
func _on_hitbox_body_entered(body: Node) -> void:
	...
	if body.has_method("take_damage") and body != owner and body != get_parent().get_parent(): # Weapon -> WeaponHolder -> Player
		...
		if body.has_method("take_damage_with_knockback"):
			var applied_knockback = Vector2.ZERO
			
			# Check Player Global Cooldown
			var p = owner as CharacterBody2D
			if not p: p = get_parent().get_parent() as CharacterBody2D
			
			if p and p.get("can_knockback"):
				applied_knockback = knockback_dir * 250.0
				p.trigger_knockback_cooldown()
				
			body.take_damage_with_knockback(final_damage, applied_knockback, is_crit)
		else:
			# Fallback anterior
