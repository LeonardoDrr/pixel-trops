...
func _die() -> void:
	...
	if anim.sprite_frames.has_animation("death"):
		...
		var duration = 1.0 # Default fallback
		if fps > 0:
			duration = float(frame_count) / fps
			
		# Account for speed_scale
		if anim.speed_scale != 0:
			duration /= abs(anim.speed_scale)
			
		# Create precise timer to cleanup exactly when anim ends
		get_tree().create_timer(duration).timeout.connect(_cleanup_boss)
