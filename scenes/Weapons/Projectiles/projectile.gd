...
func _on_body_entered(body: Node) -> void:
	...
	if body.has_method("take_damage"):
		...
		# Check Shooter Cooldown (if shooter is player)
		if shooter and shooter.get("can_knockback"):
			applied_knockback = knockback_dir * 150.0
			if shooter.has_method("trigger_knockback_cooldown"):
				shooter.trigger_knockback_cooldown()
